...
class ExtractState(TypedDict, total=False):
    ...
    session_id: str
    process_saved_posts: bool  # NEW: Flag to control saved posts processing
...
async def saved_posts_node(state: ExtractState):
    """Extract saved posts from LinkedIn - only if process_saved_posts flag is True."""
    try:
        # Skip if we're not processing saved posts
        if not state.get("process_saved_posts", False):
            print("⏭️ Skipping saved posts extraction (not requested)")
            state["saved_posts"] = []
            return state
...
async def classify_posts_node(state: ExtractState):
    """Classify saved posts - only if we have saved posts to process."""
    try:
        # Skip if we're not processing saved posts
        if not state.get("process_saved_posts", False):
            print("⏭️ Skipping classification (not requested)")
        ...
        # FIX: Pass email and password to classifier
        classifier = LinkedInPostClassifier(posts, email=email, password=password)
...
async def post_extractor_node(state: ExtractState):
    """Extract all post data using unified extractor."""
    try:
        email = state.get("email")
        password = state.get("password")
        url = state.get("url")
        ...
        state["images"] = ensure_list(extractor.data.get("images", []))
        state["videos"] = ensure_list(extractor.data.get("videos", []))
...
def create_single_post_pipeline():
    """Create pipeline for single post summarization (no saved posts)."""
    graph = StateGraph(ExtractState)

    # Add nodes for single post extraction
    graph.add_node("post", post_extractor_node)

    # Set entry and finish points
    graph.set_entry_point("post")
    graph.set_finish_point("post")

    return graph.compile()

def create_full_pipeline():
    """Create full pipeline with saved posts classification."""
    graph = StateGraph(ExtractState)
...
# Create pipelines
single_post_pipeline = create_single_post_pipeline()
...
async def summarize_post():
    ...
    try:
        ...
        # Initialize state for SINGLE POST only
        initial_state: ExtractState = {
            ...
            "classified_posts": {},
            "session_id": session_id,
            "process_saved_posts": False  # NEW: Don't process saved posts
        }

        # Run SINGLE POST pipeline only
        print(f"🔄 Running single post pipeline for {post_url}")
        final_state = await single_post_pipeline.ainvoke(initial_state)

        # Combine extracted text
        text_parts = []
        if final_state.get("text"):
            text_parts.extend(ensure_list(final_state["text"]))
        if final_state.get("images"):
            text_parts.extend(ensure_list(final_state["images"]))
        if final_state.get("documents"):
            text_parts.extend(ensure_list(final_state["documents"]))
        if final_state.get("videos"):
            text_parts.extend(ensure_list(final_state["videos"]))
        ...
        print(f"❌ Error in summarize_post: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500

@app.route("/api/process-saved-posts", methods=["POST"])
async def process_saved_posts():
    """Process and classify all saved posts."""
    try:
        data = request.json
        session_id = data.get("session_id")
        ...
        # Initialize state for SAVED POSTS processing
        initial_state: ExtractState = {
            "url": "",  # No specific URL
            "email": session["email"],
            "password": session["password"],
            ...
            "classified_posts": {},
            "session_id": session_id,
            "process_saved_posts": True  # Enable saved posts processing
        }

        # Run FULL pipeline with saved posts
        print("🔄 Running full pipeline for saved posts classification")
        final_state = await full_pipeline.ainvoke(initial_state)

        return jsonify({
            "message": "Saved posts processed successfully",
            "total_posts": len(final_state.get("saved_posts", [])),
            "classified_groups": len(final_state.get("classified_posts", {})),
            "files": [
                "saved_posts_classified.xlsx",
                "saved_posts_grouped.xlsx"
            ]
        }), 200

    except Exception as e:
        print(f"❌ Error in process_saved_posts: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
...
def index():
    ...
    return jsonify({
        ...
        "endpoints": {
            ...
            "process_saved_posts": "POST /api/process-saved-posts",
